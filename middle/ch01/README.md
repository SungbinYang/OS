...
일반적으로 프로세스가 생성될 때는 다음과 같다. exe파일을 더블 클릭하여 실행하면 운영체제는 해당 프로그램의 코드영역과 데이터 영역을 메모리에 로드하고 빈 스택과 힙 영역을 만들어 공간을 확보한다. 이제 이 프로세스를 관리하기 위한 PCB를 만들어 값을 초기화한다. 이 프로세스 생성과정은 운영체제가 부팅되고 0번 프로세스가 생성될 때 딱 한 번만 실행한다. 이제 나머지 모든 프로세스는 새로 생성하지 않고 0번 프로세스를 복사해서 사용한다. 복사는 시스템 콜의 fork() 함수를 이용한다. 이렇게 하는 이유는 새로 생성하는 것보다 복사하는 게 더 빠르기 때문이다. 0번 프로세스를 복사해서 생성되는 프로세스는 자식 프로세스라고 부르고 이 자식 프로세스 입장에서 0번 프로세스가 부모 프로세스가 된다. 자식 프로세스는 부모 프로세스의 코드, 데이터, 스택 영역과 PCB의 내용 전부를 모두 복사한다.

> Q. 0번 프로세스의 코드와 데이터를 모두 복사하여 실행하면 똑같이 0번 프로세스가 실행되는거 아닌가요?  
> A. 정답! 0번 프로세스를 그대로 복사하니 당연한 결과이다.

위의 질의응답을 보고 생각을 하면 "어떻게 프로세스의 고유의 코드를 어떻게 실행할까?"라는 생각이 떠오르게 된다. 정답은 바로 exec() 함수를 이용하는 것이다. fork() 함수로 프로세스를 복사 후 exec() 함수를 실행시켜 부모를 복사한 자식 프로세스의 코드와 데이터 영역을 원하는 값으로 덮어쓸 수 있다. 그러면 이 때부터 자식 프로세스는 부모 프로세스와 완전히 다르게 동작한다.

그러면 이제 코드를 통해 자세히 살펴보자.
...
위의 코드를 한번 보자. 6번 라인에 fork()함수를 호출한다. 그러면 운영체제는 이 프로세스와 동일한 프로세스를 복사하고 반환 값으로 부모 프로세스는 0이 아닌 값, 자식 프로세스는 0을 반환한다. 이 2개의 프로세스는 CPU 스케줄링에 따라 어떤 프로세스가 먼저 실행할지 운영체제가 결정한다. 만약 부모 프로세스가 먼저 실행되었다고 해보자. 그러면 pid값은 부모 프로세스이니 0이 아닌 값 즉, 1이 나왔다고 해보자. 그러면 8번 라인 조건문에 거짓이므로 11번 라인에 else문으로 가게 된다. 그리고 wait() 시스템 함수를 호출한다. 이 함수는 자식 프로세스로부터 exit status값이 전달되기 전까지 아무것도 수행 안 하고 대기한다. 그리고 컨텍스트 스위칭을 거쳐 자식 프로세스로 포커스가 갔다고 해보자. 자식 프로세스의 pid값은 0이 나왔으므로 8번 라인 조건에 참이 되고 execlp() 함수로 인터넷 브라우저를 실행시킨다. 그리고 인터넷 브라우저를 종료하면 exit() 함수를 호출하고 status값으로 0을 전달한다. 그러면 부모 프로세스로 컨텍스트 스위칭이 되면 부모 프로세스는 자식 프로세스로부터 exit status값을 받았으므로 자식 프로세스를 완전히 종료시키고 다음 라인의 코드를 실행한다. 마지막으로 exit()을 호출함으로 부모 프로세스도 종료한다.

만약 부모 프로세스가 자식 프로세스보다 먼저 종료되거나 자식 프로세스가 비정상적으로 종료가 되어서 exit()을 호출하지 못할 경우, 메모리에 프로세스가 좀비처럼 계속 살아있는데 이를 좀비 프로세스라고 부른다. PC를 오래 켜두면 느려지는 현상이 이 좀비 프로세스때문이다. 이런 경우, PC를 재부팅하면 해결이 된다.

> 이 포스트는 [감자님 강의](https://www.inflearn.com/course/%EB%B9%84%EC%A0%84%EA%B3%B5%EC%9E%90-%EC%9A%B4%EC%98%81%EC%B2%B4%EC%A0%9C/dashboard '인프런 강의')를 기반으로 작성되었습니다.

## 쓰레드

운영체제가 작업을 처리하는 단위는 프로세스이다. 사용자가 운영체제에게 작업을 요구하면 그만큼 프로세스의 수도 증가를 한다. 프로세스가 생성이 되면 PCB를 생성하고 메모리에 코드, 데이터, 스택, 힙 영역을 생성하게 된다. 프로세스의 수가 많아지면 그 만큼 PCB, 데이터, 스택, 코드, 힙 영역을 만들어줘야 하기 때문에 너무 무거워진다.

예를 들어보자. 크롬 브라우저를 실행시키면 프로세스 1개가 생성이 된다. 여기서 탭을 추가하면 기존 프로세스를 복사해서 2개의 프로세스가 존재하게 된다. 만약, 100개의 탭을 열었다면 100개의 프로세스가 생성이 될 것이고 크롬이 메모리를 너무 많이 차지할 것이다. 또한 브라우저 탭 간 통신을 하려면 IPC를 이용해야 하는데 IPC는 통신의 비용이 상대적으로 많이 들었다. 이래서 컴퓨터 시스템 개발자들은 이 문제를 어떻게 해결할까 고민한 결과, 쓰레드가 나오게 되었다. 쓰레드는 프로세스 내에 1개 이상 존재한다. 한 프로세스 내 쓰레드들은 그 프로세스의 PCB, 코드, 데이터, 힙 영역을 공유할 수 있고 스택영역은 쓰레드 개별로 가지게 된다. 프로세스 내에 여러개의 쓰레드가 있으니 이 쓰레드를 구분하기 위해 쓰레드 ID가 생겼고 이를 통제 및 관리하기 위한 TCB가 생겼다.

이제 운영체제 관점에서 쓰레드를 구분할 수 있게 되었고 작업 처리 단위는 쓰레드로 변경되었다. 예를 다시 들어보자. 브라우저를 실행하면 프로세스 1개와 쓰레드 1개가 생성된다. 탭을 열면 쓰레드 1개가 더 추가된다. 100개의 탭을 열면 쓰레드 100개와 프로세스 1개가 되는데 이는 너무 추상적이므로 실제 예시를 한번 들어보자. 크롬 브라우저와 파이어폭스 브라우저를 비교해 보자. 크롬 브라우저를 실행하면 프로세스 1개가 생성되고 탭이 추가할 때마다 프로세스를 생성하는 방식이라면 파이어폭스는 탭 4개까지만 프로세스를 생성하고 그 이후는 쓰레드를 증가시키는 방식으로 개발되었다. 쓰레드로 인하여 코드, 데이터, 힙 영역이 공유가 되니 메모리도 절약을 하게 되었다.

### 프로세스, 쓰레드 장단점

- 안전성: 프로세스는 독립적이기 때문에 하나의 프로세스가 문제가 생기더라도 다른 프로세스에게는 영향을 끼치지 않는다. 하지만 쓰레드는 1개의 프로세스 내에 존재하기 때문에 해당 프로세스가 문제가 생기면 모든 쓰레드들이 문제가 생기므로 안정성 측면에서는 쓰레드 방식보단 프로세스 방식이 우월하다.
- 속도와 자원: 각각 프로세스는 서로 고유한 자원을 가진다. 코드, 데이터, 힙, 스택 전부 따로 두어서 프로세스 간 통신을 하려면 IPC를 이용해야 하기 때문에 오버헤드가 너무 크고 속도도 느렸다. 반면에 쓰레드는 한 프로세스 내에서 스택을 제외한 나머지 영역을 모두 공유하기 때문에 오버헤드가 굉장히 작고 쓰레드간 통신도 쉽게 가능하다. 하지만 공유되는 공간의 문제(동시성)가 발생하기도 한다.
