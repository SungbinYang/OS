...
CPU가 입출력 장치와 통신을 하려고 한다. CPU는 입출력 작업이 들어오면 입출력 관리자에게 입출력 명령을 내린다. CPU관점에서는 입출력 명령이 언제 완료되는지를 알 수 없기에 주기적인 확인이 필요하다. 이런 방식을 **polling 방식**이라고 한다. polling 방식의 단점은 주기적으로 CPU가 확인해줘야 하기 때문에 성능에 좋지 않다.

인터럽트는 이런 polling 방식의 단점을 해결한 방식이다. CPU가 입출력 관리자에게 입출력 명령을 내리고 다른 작업을 한다. 그리고 입출력 관리자는 입출력이 완료되면 CPU에게 인터럽트 신호를 보내고 그 신호를 받은 CPU는 인터럽트 서비스 루틴(ISR)을 실행한다. 인터럽트는 비동기적으로 동작하기 때문에 성능상 이점이 있다.

> 인터럽트 서비스 루틴: 특정 인터럽트가 들어오면 그 인터럽트를 처리하는 함수이다.

인터럽트는 하드웨어 방식, 소프트웨어 방식 2가지로 나눈다. 하드웨어 방식은 위에서 설명한 입출력등과 같은 인터럽트 방식이고 소프트웨어 방식은 프로그램에서 발생한 인터럽트로 유효하지 않는 메모리 접근, 0으로 나누는 행위에 해당된다.
